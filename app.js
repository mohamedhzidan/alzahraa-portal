...
(function (global) {
  ...
  function buildDemoChips() {
    var host = document.getElementById('demoAccounts');
    if (!host) return;
    var picks = ['admin', 'gm', 'finance', 'accountant', 'purchase', 'store', 'pm', 'technical', 'hr', 'legal', 'it', 'auditor', 'employee'];
    host.innerHTML = '';
    Auth.users().forEach(function (u) {
      if (picks.indexOf(u.username) === -1) return;
  ...
  function wireShell() {
    ...
    /* restore file input */
    document.getElementById('restoreInput').addEventListener('change', doRestore);
    ...
    document.addEventListener('keydown', function (e) {
      ...
      if (e.key === 'Escape') {
        ...
        else if (!document.getElementById('modalHost').hidden) UI.closeModal();
        else closeSidebar();
      }
    });
  ...
  function backup() {
    ...
    var name = 'alzahraa-backup-' + new Date().toISOString().slice(0, 10) + '.json';
    UI.downloadJSON(dump, name);
    Store.log('backup', 'system', '', name);
    UI.toast(t('set.backupOk'));
  }
  function restorePrompt() { document.getElementById('restoreInput').click(); }
  function doRestore(e) {
    ...
    reader.onload = function () {
      try {
        ...
        if (!Store.importAll(dump)) throw new Error('bad');
        UI.toast(t('set.restoreOk'));
        setTimeout(function () { location.reload(); }, 700);
      } catch (err) {
        UI.toast(t('set.restoreBad'), 'error');
      }
    };
  ...
  function updateStorage() {
    ...
    if (txt) txt.textContent = (u.bytes / 1024).toFixed(0) + ' KB / 5 MB';
  }
