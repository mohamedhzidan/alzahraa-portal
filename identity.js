...
(function (global) {
  ...
  /* كلمات مرور سهلة القراءة من ورقة مطبوعة وسهلة الكتابة على الموبايل،
     لكنها غير قابلة للتخمين. النمط: كلمة + رمز + ٤ أرقام */
  ...
  /* ------------------------------------------------------------------
     ...
     ------------------------------------------------------------------ */
  function createFor(employeeId, role, opts) {
    opts = opts || {};
    ...
    var user = Store.create('users', {
      ...
      mustChangePassword: SETTINGS.forceChangeOnFirstLogin,
  ...
  function changeOwnPassword(oldPass, newPass) {
    ...
    if (String(u.password) !== String(oldPass).trim()) {
      return { ok: false, error: L({ ar: 'كلمة المرور الحالية غير صحيحة', en: 'Current password is incorrect' }) };
    }
    var p = String(newPass || '').trim();
    if (p.length < SETTINGS.minPasswordLength) {
      return { ok: false, error: L({
        ar: 'كلمة المرور يجب ألا تقل عن ' + SETTINGS.minPasswordLength + ' حروف/أرقام',
        en: 'Password must be at least ' + SETTINGS.minPasswordLength + ' characters' }) };
    }
    if (p === String(oldPass)) {
      return { ok: false, error: L({ ar: 'اختر كلمة مرور مختلفة عن الحالية', en: 'Choose a password different from the current one' }) };
    }
    Store.save('users', u.id, {
      password: p, mustChangePassword: false, passwordSetAt: new Date().toISOString()
    });
    u.password = p; u.mustChangePassword = false;
  ...
  function applyMigration(domain, overwrite) {
    ...
    Store.all('users').forEach(function (u) {
      if (u.email && !overwrite) return;
      Store.save('users', u.id, { email: u.username + '@' + d }, { silent: true });
      n++;
    });
    Store.setMeta({ companyDomain: d });
    SETTINGS.companyDomain = d;
    Store.log('migrate_email', 'users', '', d, n + ' accounts');
    return { ok: true, count: n, domain: d };
  }
  ...
  function slips(userIds, passwords) {
    ...
    (userIds || []).forEach(function (uid) {
      ...
      if (!u) return;
      var pw = (passwords && passwords[uid]) || u.password || '—';
  ...
  function promptPasswordChange(force) {
    ...
    var body =
      ...
      '<p class="field-hint mt-1">' +
        L({ ar: 'لا تقل عن ' + SETTINGS.minPasswordLength + ' حروف. لا تستخدم كلمة مرور تستعملها في مكان آخر.',
            en: 'At least ' + SETTINGS.minPasswordLength + ' characters. Do not reuse a password from elsewhere.' }) +
      '</p>';
