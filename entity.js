...
(function (global) {
  ...
  function openForm(moduleId, id, presetData) {
    ...
    groupFields(mod.fields).forEach(function (sec) {
      ...
      sec.fields.forEach(function (f) { body += fieldHTML(f, draft); });
      body += '</div></div>';
    });
  ...
  function fieldHTML(f, rec) {
    ...
    if (f.type === 'textarea') {
      h += '<textarea class="textarea" name="' + UI.attr(f.name) + '"' + (f.readonly ? ' disabled' : '') + '>' + UI.esc(v || '') + '</textarea>';
    } else if (f.type === 'select') {
      h += '<select class="select" name="' + UI.attr(f.name) + '"' + (f.readonly ? ' disabled' : '') + '>';
      ...
      h += '<input type="' + type + '" class="input" name="' + UI.attr(f.name) + '"' + step +
        (f.readonly ? ' disabled' : '') + ' value="' + UI.attr(v === undefined || v === null ? '' : v) + '">';
    }
