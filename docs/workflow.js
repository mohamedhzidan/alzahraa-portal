...
(function (global) {
  ...
  function isPosted(rec) { return rec && (rec.status === 'approved' || rec.status === 'reversed'); }
  ...
  async function transition(moduleId, recId, action, reason) {
    ...
    /* Fast client-side feedback. The database repeats every check and is the
       final authority, so bypassing the browser cannot bypass the workflow. */
    switch (action) {
      case 'submit':
    ...
    var rpc = await Auth.client().rpc('az_transition_document', {
