/* =========================================================================
   ...
   -------------------------------------------------------------------------
   يعمل الآن بلا إنترنت وبلا اشتراك وبلا مفتاح — يحسب الإجابات من بياناتك.
   وعندما تُنشئ قاعدة البيانات يمكن توصيله بنموذج ذكاء اصطناعي حقيقي
   دون إعادة بناء أي شيء (اقرأ قسم AI_PROVIDER بالأسفل).

   Works today with no internet, no subscription and no API key — it computes
   answers from your own data. Once the database exists it can be connected to
   a real AI model without rebuilding anything.
...
(function (global) {
  ...
  /* ═══════════════════════════════════════════════════════════════════
     AI_PROVIDER — المرحلة الثانية
     اتركه 'local' الآن. بعد إنشاء قاعدة البيانات ودالة Edge Function،
     غيّره إلى 'edge' وضع الرابط، وسيتحوّل المساعد لمحادثة ذكية كاملة
     مع الحفاظ على كل ما بالأسفل كمصدر للحقائق.
     ═══════════════════════════════════════════════════════════════════ */
  var AI_PROVIDER = 'local';        /* 'local' | 'edge' */
  var EDGE_URL = '';                /* مثال: https://xxx.supabase.co/functions/v1/assistant */
  ...
  function render(host) {
    ...
    html += '<div class="alert alert-info">' + UI.icon('eye', 17) + '<span>' +
      L({ ar: 'المساعد يقرأ بياناتك أنت فقط ويحسب الإجابة لحظياً — لا يرى بيانات زملائك، ولا يعتمد ولا يعدّل أي مستند. ' +
              'القرار دائماً لك.',
          en: 'The assistant reads only your data and computes answers live. It never sees colleague data and never ' +
              'approves or edits anything. The decision is always yours.' }) + '</span></div>';
    ...
    function ask(q) {
      ...
      res.forEach(function (l) { h += '<div>' + UI.esc(l) + '</div>'; });
      h += '</div></div></div>';
