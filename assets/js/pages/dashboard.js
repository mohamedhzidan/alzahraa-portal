...
(function (global) {
  ...
  function approved(table) {
    return Store.all(table).filter(function (r) { return r.status === 'approved' && !r.isReversal; });
  }

  /* Posted ledgers retain the original reversed row and its separately
     approved negative counter-document, so their net effect is zero. */
  ...
  function actualCost(projectId) {
    var sum = 0;
    posted('stockIssues').forEach(function (r) { if (!projectId || r.project === projectId) sum += Number(r.subTotal) || 0; });
    posted('supplierInvoices').forEach(function (r) { if (!projectId || r.project === projectId) sum += Number(r.subTotal) || 0; });
    posted('subIPCs').forEach(function (r) { if (!projectId || r.project === projectId) sum += Number(r.currentWork) || 0; });
    Store.all('equipmentLogs').forEach(function (r) { if (!projectId || r.project === projectId) sum += Number(r.cost) || 0; });
    posted('journal').forEach(function (r) { if (projectId && r.project === projectId) sum += Number(r.totalDebit) || 0; });
    return sum;
  }

  function committed(projectId) {
    var sum = 0;
    posted('purchaseApprovals').forEach(function (r) {
      if (projectId && r.project !== projectId) return;
      sum += Number(r.grandTotal) || 0;
    });
    /* subtract what has already turned into an invoice */
    posted('supplierInvoices').forEach(function (r) {
  ...
  function costByType() {
    ...
    posted('stockIssues').forEach(function (r) { addBy(r.costItem, Number(r.subTotal) || 0); });
    posted('supplierInvoices').forEach(function (r) { addBy(r.costItem, Number(r.subTotal) || 0); });
    posted('subIPCs').forEach(function (r) { addBy(r.costItem, Number(r.currentWork) || 0); });
  ...
  function receivable() {
    var inv = 0, col = 0;
    posted('clientIPCs').forEach(function (r) { inv += Number(r.netDue) || 0; });
    posted('receipts').forEach(function (r) { col += Number(r.amount) || 0; });
    return Math.max(0, inv - col);
  }
  function payable() {
    var inv = 0, paid = 0;
    posted('supplierInvoices').forEach(function (r) { inv += Number(r.grandTotal) || 0; });
    posted('subIPCs').forEach(function (r) { inv += Number(r.netDue) || 0; });
    posted('payments').forEach(function (r) { paid += Number(r.amount) || 0; });
  ...
  function cashBalance(accId) {
    ...
    posted('receipts').forEach(function (r) { if (r.cashAccount === accId) bal += Number(r.amount) || 0; });
    posted('payments').forEach(function (r) { if (r.cashAccount === accId) bal -= Number(r.amount) || 0; });
    return bal;
  }
